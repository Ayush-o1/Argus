...
| `REDIS_PORT` | `6379` | docker-compose | Host port the `redis` service publishes on. Same reasoning as `POSTGRES_PORT`: a machine may already run its own Redis on 6379, so this is configurable rather than hardcoded. Update `REDIS_URL`'s port to match if you change it. |
...
| `POSTGRES_HOST` / `POSTGRES_PORT` | `localhost` / `55432` | backend, docker-compose | Where ARGUS's PostgreSQL listens. The port is deliberately not 5432 — many machines already run their own PostgreSQL, and competing for the default port produces authentication errors that read as credential bugs. To use a server you already run, point these at it instead of starting the compose service; see [LOCAL_TESTING.md](LOCAL_TESTING.md#using-a-postgresql-you-already-run). |
| `POSTGRES_DB` | `argus` | backend, docker-compose | Database name. ARGUS never creates it — `CREATE DATABASE` it yourself, then let the migrations build the schema inside it. |
