...
- **Ingestion** — connectors fetch; everything after fetching is shared pipeline. A record lands raw and hash-keyed, is validated and normalised against the connector's declared mapping, and becomes an observation. Anything rejected goes to a dead-letter queue with the stage and reason, never silently dropped. Runs are **durable queued jobs** in Postgres (`SELECT ... FOR UPDATE SKIP LOCKED`), so a restart postpones a batch rather than losing it — and a job can be enqueued in the same transaction as the change that motivates it, which no external broker can offer. Adding a source is a database row; adding a *kind* of source is one class and a registry entry.
- **Entity resolution** — the same real-world thing reported twice becomes one entity, *reversibly*. Blocking narrows the population to pairs worth comparing; a weighted, per-attribute model scores each pair and carries the share of evidence that was actually comparable alongside the score. The matcher merges on its own only where an identifier matched exactly, nothing disagreed, and enough of the model was comparable to mean anything — everything else is queued for a person. **The decision ledger in PostgreSQL is the record; the `SAME_AS` edge in Neo4j is a projection that can be dropped and re-derived**, which is what makes a merge reversible without restoring anything. Clusters are the connected components of active merges, and a component containing a pair judged *different* is marked contested rather than resolved.
