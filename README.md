...
- **Patterns over time and space** — "is this increase significant?" is answered with an exact Poisson rate ratio, a confidence interval and a p-value against a stated prior period, so a doubling from four events to nine is correctly reported as ordinary. Trend is Mann-Kendall with Sen's slope, weekly rhythm is a chi-square against an even week, and each declines to answer where the data cannot support one. Concentrations are found by density clustering on great-circle distance rather than by grouping on a country string — which is how a cluster straddling a border shows up as one finding instead of two unremarkable national totals — and Getis-Ord Gi* says whether a concentration is more than the map being dense there anyway, with the multiple-comparison correction applied and both verdicts published.
